...
def main():
    ...
    parser.add_argument("--min-clean-ratio-score", type=float, default=MIN_CLEAN_RATIO_SCORE)
    parser.add_argument("--min-spike-score", type=float, default=MIN_SPIKE_SCORE)
    parser.add_argument("--min-spike-periods", type=int, default=MIN_SPIKE_PERIODS)
    parser.add_argument("--spike-time-grain", choices=["monthly", "yearly"], default="monthly")
    args = parser.parse_args()

    spark = (
        SparkSession.builder
        .appName("BuildSlangCandidatesLocal")
        .master("local[*]")
        .getOrCreate()
    )
    ...
    if args.mode == "global":
        ...
        spike_scores_dir = str(out_dir / f"spike_scores_{args.spike_time_grain}_global_parquet")

        clean_ratio_candidates_parquet_dir = str(
            out_dir / "slang_candidates_clean_ratio_global_parquet"
        )
        clean_ratio_candidates_csv_dir = str(
            out_dir / "slang_candidates_clean_ratio_global_csv"
        )

        clean_ratio_spiking_candidates_parquet_dir = str(
            out_dir / f"slang_candidates_clean_ratio_spiking_{args.spike_time_grain}_global_parquet"
        )
        clean_ratio_spiking_candidates_csv_dir = str(
            out_dir / f"slang_candidates_clean_ratio_spiking_{args.spike_time_grain}_global_csv"
        )
        ...
        spike_scores_dir = str(out_dir / f"spike_scores_{args.spike_time_grain}_subreddit_parquet")

        clean_ratio_candidates_parquet_dir = str(
            out_dir / "slang_candidates_clean_ratio_subreddit_parquet"
        )
        clean_ratio_candidates_csv_dir = str(
            out_dir / "slang_candidates_clean_ratio_subreddit_csv"
        )

        clean_ratio_spiking_candidates_parquet_dir = str(
            out_dir / f"slang_candidates_clean_ratio_spiking_{args.spike_time_grain}_subreddit_parquet"
        )
        clean_ratio_spiking_candidates_csv_dir = str(
            out_dir / f"slang_candidates_clean_ratio_spiking_{args.spike_time_grain}_subreddit_csv"
        )
    ...
    # -------------------------
    profanity = (
        spark.read.text(str(data_dir / "lexicons/profanity.txt"))
    ...
    typos = (
        spark.read.text(str(data_dir / "lexicons/typos.txt"))
        .select(col("value").alias("token"))
        .filter(col("token") != "")
        .distinct()
    ...
    reddit_jargon = (
        spark.read.text(str(data_dir / "lexicons/reddit_jargon.txt"))
        .select(col("value").alias("token"))
        .filter(col("token") != "")
        .distinct()
